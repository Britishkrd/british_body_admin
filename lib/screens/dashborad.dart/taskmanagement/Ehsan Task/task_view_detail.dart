...
class _TaskViewDetailState extends State<TaskViewDetail> {
  ...
  for (var doc in tasksSnapshot.docs) {
    ...
    // Check for approved absences that overlap with the task's deadline
    ...
    bool hasApprovedAbsence = false;
    for (var absenceDoc in absenceSnapshot.docs) {
      final absenceStart = (absenceDoc['start'] as Timestamp).toDate();
      final absenceEnd = (absenceDoc['end'] as Timestamp).toDate();

      // Check if the task's deadline falls within the absence period
      if (deadline.isAfter(absenceStart.subtract(const Duration(days: 1))) &&
          deadline.isBefore(absenceEnd.add(const Duration(days: 1)))) {
    ...
    if (hasApprovedAbsence) {
      // Delete the task if there's an approved absence
      await FirebaseFirestore.instance
          .collection('user')
          .doc(widget.email)
          .collection('tasks')
          .doc(taskId)
          .delete();
    } else if (deadline.isBefore(DateTime.now()) && doc['status'] == 'pending') {
      // Only update to 'unfinished' and apply punishment if no approved absence
      await _updateTaskStatus(
        taskId,
        'unfinished',
        deductionAmount: deductionAmount,
